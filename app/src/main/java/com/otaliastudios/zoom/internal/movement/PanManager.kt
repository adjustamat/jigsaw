...
internal class PanManager(
    ...
    private val provider: () -> MatrixController
) {
    ...
    private val controller get() = provider()
    ...
    val isOverEnabled get() = horizontalOverPanEnabled || verticalOverPanEnabled

    /** whether pan is enabled, horizontally or vertically */
    val isEnabled get() = horizontalPanEnabled || verticalPanEnabled
    ...
    internal fun computeStatus(horizontal: Boolean, output: Status) {
        ...
        @ZoomApi.ScaledPan val contentDim =
            (if (horizontal) controller.contentScaledWidth else controller.contentScaledHeight).toInt()
...
//        @Suppress("CascadeIf")
    ...
    internal fun checkBounds(horizontal: Boolean, allowOverScroll: Boolean): Float {
        ...
        @ZoomApi.ScaledPan val contentSize =
            if (horizontal) controller.contentScaledWidth else controller.contentScaledHeight
